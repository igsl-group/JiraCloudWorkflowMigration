...
public class RestUtil<T> {
	...
	private List<Integer> statusList = Arrays.asList(HttpStatus.SC_OK);
	 ...
	 */
	public RestUtil<T> config(Config config) throws UnsupportedEncodingException {
		return this
				.scheme(config.getScheme())
				.authenticate(config.getEmail(), config.getToken())
				.throttle(config.getRate(), config.getPeriod());
	}
